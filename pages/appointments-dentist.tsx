...
import 'react-toastify/dist/ReactToastify.css';
  ...
  export default function Home( ){
    ...
    const sortedAppointment = appointments.filter((appointment) => {
    ...
    console.log('Filtered Users:', sortedAppointment);
    ...
    useEffect(() => {
        ...
        try {
            const response = await fetch('api/dentist/appointment-files/appointments');
            if (!response.ok) {
                ...
                <table className={styles.table2}>
                    ...
                    <tbody>
                        {filteredBySelectedFilters.length > 0
                            ...
                            : sortedAppointment.map((appointment: any, index: any) => (
                            <tr key={appointment._id}>
                            <td>{index + 1}</td>
                            <td>{appointment.patientName}</td>
                            <td>{new Intl.DateTimeFormat('en-US', options).format(new Date(appointment.date))}</td>
