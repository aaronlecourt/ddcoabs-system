    ...
    fetch(`/api/dentist/accounts/role`, {
      ...
      .then(async (response) => {
        if (!response.ok) {
          ...
          toast.error('User role update failed!');('Update failed: ' + JSON.stringify(error));
        } else {
  ...
  const archiveUser = async (e: any) => {
    ...
    fetch(`/api/dentist/accounts/archive`, {
      ...
      .then(async (response) => {
        if (!response.ok) {
          const error = await response.json();
          toast.error('Update failed: ' + JSON.stringify(error)); //notif
        } else {
          const updatedUser = await response.json();
          toast.success('User archived successfully!'); //notif
          ...
          // Remove the updated user from the table
          setUsers((prevUser) =>
            prevUser.filter((user) => user._id !== updatedUser._id)
          );
          
        }
      })
      .catch((error) => {
        toast.error('Update failed'); //notif
