...
import { isServiceFormValid } from "../validations/add-service";
...
export default function Services() {
  ...
  const [updateServiceFormData, setUpdateServiceFormData] =
    useState<UpdateServicesFormData>({
      _id: "",
      name: "",
      price: "",
      description: "",
  ...
  const totalPages = Math.max(
    Math.ceil(searchedServices.length / itemsPerPage),
    1
  );
  ...
  const handleFilterChange = async (filter: any) => {
    ...
    try {
      const response = await fetch(
        `api/dentist/dentist-service?filter=${filter}`
      );
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const response = await fetch(
          `api/dentist/dentist-service${queryParams}`
        );
  ...
  const addService = async (e: any) => {
    ...
    const isValid = isServiceFormValid(
      serviceFormData,
      errorFormData,
      setErrorFormData
    );
        ...
        if (!response.ok) {
          ...
          switch (selectedFilter) {
            ...
            case "Latest to Oldest":
              ...
              break;
            case "Alphabetical (A-Z)":
              updatedSortedServices.sort((a, b) =>
              ...
              {pageItems.map((service, index) => (
                <tr key={service._id}>
                  ...
                  <td className={styles1.tableAction}>
                    ...
                    <ArchiveButton
                      onClick={() => onUpdateService(service, "archive")}
                    >
                      <FontAwesomeIcon
                        icon={faFileArchive}
                        width={24}
    ...
    return (
      <>
        ...
        <Modal
          ...
          modalRadius={10}
          onClose={() => {
            setErrorFormData(initialErrorState);
            setShowAddService(false);
        ...
        >
          ...
          <label> Type: </label>
          {errorFormData.type.error && (
            <span className={styles.errorMessage}>
              {errorFormData.type.message}
            </span>
          )}
          <div className={styles1.cancelText}>
            <div className={styles1.filters__sortDropdown}>
...
<div className={`${styles1.cancelText} ${errorFormData.name.error ? 'errorInput' : ''}`}>
  <div className={styles1.filters__search}>
    <input
      type="text"
      name="name"
      className={`${styles1.filters__searchInput} ${errorFormData.name.error ? 'errorInput' : ''}`}
      value={updateServiceFormData.name}
      onKeyDown={(e) => handleFormEnter(e, updateService)}
      onChange={(e) =>
...
<div className={`${styles1.cancelText} ${errorFormData.price.error ? 'errorInput' : ''}`}>
  <div className={styles1.filters__search}>
    <input
      type="text"
      name="price"
      className={`${styles1.filters__searchInput} ${errorFormData.price.error ? 'errorInput' : ''}`}
      value={updateServiceFormData.price}
      onChange={(e) =>
        handleFormDataChange(
          e,
          setUpdateServiceFormData,
...
<label> Type: </label>
<div className={`${styles1.cancelText} ${errorFormData.type.error ? 'errorInput' : ''}`}>
  <div className={styles1.filters__sortDropdown}>
    <select
      onChange={(e: any) =>
        setUpdateServiceFormData({
      ...
      className={`${errorFormData.type.error ? 'errorOption' : ''}`}
    >
      ...
      {types.map((type, index) => (
        <option key={index} value={type}>
          ...
          <div className={styles1.cancelText}>
            <div className={styles1.filters__search}>
              <input
                ...
                name="description"
                className={styles1.filters__searchInput}
                value={updateServiceFormData.description}
                onChange={(e) =>
                  handleFormDataChange(
                    e,
                    setUpdateServiceFormData,
          ...
          <div className={styles1.cancelActions}>
            <CancelButton onClick={() => setShowUpdateService(false)}>
              Cancel
            </CancelButton>
            <Button onClick={updateService} type="submit">
              Update
            </Button>
          </div>
        </Modal>
        ...
        {/* ARCHIVE SERVICE */}
        <Modal
          open={showArchiveService}
          setOpen={setShowArchiveService}
          modalWidth={400}
          modalRadius={10}
        >
          ...
          <input type="hidden" name="_id" value={updateServiceFormData._id} />
          <div className={styles1.cancelActions}>
            <CancelButton onClick={() => setShowArchiveService(false)}>
              No
            </CancelButton>
            <Button onClick={archiveService} type="submit">
              Yes
            </Button>
          </div>
        </Modal>
